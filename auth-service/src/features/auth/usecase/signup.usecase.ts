...
export class SignupUseCase implements UseCase<SignupUseCaseRequest, SignupUseCaseResponse> {
    async execute(request: SignupUseCaseRequest, _requestContext?: RequestContext, _response?: Response): Promise<Result<SignupUseCaseResponse>> {
        const connection = AppDataSource.manager;

        const userDetailsRepository = new UserDetailsRepositoryDb(connection);

        const userDetailsInfo = await userDetailsRepository.findBy_Email_or_Username_or_PhoneNumber(request.email, request.userName, request.phoneNumber);
